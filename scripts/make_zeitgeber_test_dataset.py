...
FS = vz.FS  # 1000 Hz
# 8 channels (no Hip — the manual channel filter rejects LHip/RHip). These raw names
# map to canonical abbreviations via the dataset `channels` config written below.
...
def main():
    ...
    samples = {
        ...
        "channels": {
            "LMot": ["L Mot"], "RMot": ["R Mot"],
            "LBar": ["L Bar"], "RBar": ["R Bar"],
            "LAud": ["L Aud"], "RAud": ["R Aud"],
            "LVis": ["L Vis"], "RVis": ["R Vis"],
        },
        "animals": animals,
    }
