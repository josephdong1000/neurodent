...
class TestChannelAveraging:
    ...
    def test_get_channel_averaged_result_integration(self, mock_war_full):
        ...
        # Note: zpcorr is a simple matrix feature (no bands), zcohere/zimcoh are banded matrix features
        expected_cols = [
            'timestamp', 'genotype', 'animalday',  # Metadata
            'logrms',  # Linear feature
            'zpcorr',  # Simple matrix feature (no bands, just averaged)
    ...
    def test_list_format_matrix_features(self, mock_war_with_matrix_lists):
        """Test that simple matrix features (zpcorr) stored as lists are converted to arrays"""
        ...
        # After averaging: zpcorr should be a single scalar column (not split into bands)
        assert 'zpcorr' in df_averaged.columns, "Expected column zpcorr"
        assert isinstance(df_averaged['zpcorr'].iloc[0], (int, float, np.number))
        assert not isinstance(df_averaged['zpcorr'].iloc[0], (np.ndarray, list))

        # Should NOT have band columns for zpcorr (it's not a banded feature)
        for band in constants.BAND_NAMES:
            col_name = f'zpcorr_{band}'
            assert col_name not in df_averaged.columns, f"Should not have banded column {col_name}"
