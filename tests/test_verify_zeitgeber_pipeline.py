...
def test_verify_pipeline():
    ...
    # 2. Time columns
    assert "zt_minutes" in df_processed.columns
    ...
    assert "zt_minutes" not in feature_cols
    
    logger.info("Metadata helpers passed")
    
    # Check expansion
    # Original unique zt_minutes should be 24 (hourly bins)
    ...
    assert df_plot["zt_minutes"].max() >= 1440
