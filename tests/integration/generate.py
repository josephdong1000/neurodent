...
# ---------------------------------------------------------------------------
# EDF format generator
# ---------------------------------------------------------------------------

# All 10 default Intan channel IDs (matching DEFAULT_ID_TO_NAME).
ALL_CHANNEL_IDS = [9, 10, 12, 14, 15, 16, 17, 19, 21, 22]
...
def _write_edf_file(
    ...
    *,
    ...
    """Write a single EDF file with synthetic EEG data.

    Channel labels use the Intan-style ``{port}-{id:03d}`` format (e.g.
    ``C-009``) so that ``parse_chname_to_abbrev(..., assume_from_number=True)``
    can map them to standard channel abbreviations.

    Args:
        filepath: Output ``.edf`` path (parent directories created if absent).
        n_channels: Number of EEG channels.
        sampling_rate: Sampling rate in Hz.
        duration_s: Recording duration in seconds.
        channel_ids: Intan channel IDs for labels.  Defaults to the first
            *n_channels* entries of :data:`ALL_CHANNEL_IDS`.
        port_letter: Intan port letter prefix (e.g. ``"C"`` or ``"D"``).
        seed: NumPy RNG seed for reproducibility.

    Returns:
        Dict with ``"edf_path"`` and ``"n_samples"``.
    """
    import pyedflib

    filepath = Path(filepath)
    filepath.parent.mkdir(parents=True, exist_ok=True)
    channel_ids = channel_ids or ALL_CHANNEL_IDS[:n_channels]
    rng = np.random.default_rng(seed)
    ...
    channel_labels = [f"{port_letter}-{cid:03d}" for cid in channel_ids]

    with pyedflib.EdfWriter(str(filepath), n_channels, file_type=pyedflib.FILETYPE_EDFPLUS) as f:
        for ch in range(n_channels):
            f.setLabel(ch, channel_labels[ch])
            f.setSamplefrequency(ch, sampling_rate)
            f.setPhysicalMaximum(ch, data[ch].max())
            f.setPhysicalMinimum(ch, data[ch].min())
            f.setDigitalMaximum(ch, 32767)
            f.setDigitalMinimum(ch, -32768)
            f.setPhysicalDimension(ch, "uV")
        f.writeSamples(data)

    return {"edf_path": filepath, "n_samples": n_samples}
