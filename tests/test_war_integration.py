...
class TestWARIntegration:
    ...
    def test_war_spike_integration_via_read_sars_spikes(self, animal_organizer_with_war):
        ...
        assert all(isinstance(val, (list, np.ndarray)) for val in nspike_data), "nspike should be list or array of counts per channel"

        # Check lognspike values - they should be numpy arrays after processing
        lognspike_data = enhanced_war.result['lognspike']
        assert all(isinstance(val, (list, np.ndarray)) for val in lognspike_data), "lognspike should be list or array of log-transformed counts"
        ...
        logging.info(f"Successfully integrated spike features. "
    ...
    def test_spike_counts_consistency(self, animal_organizer_with_war):
        ...
        for i, fdsar in enumerate(fdsar_list):
            ...
            logging.info(f"Spike count consistency check for {animalday}: "
                        f"original={original_total}, war={war_total}")
