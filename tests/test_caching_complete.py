...
class TestMNECachingOptimization:
    ...
    def test_mne_cache_prevents_loading_when_cache_exists(self, mock_extract_func):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch('pythoneeg.core.core.se.read_edf') as mock_read_edf, \
                 patch('pythoneeg.core.core.DDFBinaryMetadata.from_json') as mock_from_json:
            ...
            for cache_policy, should_call_extract in policies_and_expected_calls:
                ...
                with patch('pythoneeg.core.core.mne.export.export_raw') as mock_export, \
                    ...
                    if should_call_extract:
                        assert mock_func.call_count >= 1, f"Cache policy '{cache_policy}' should have called extract_func"
                        assert mock_export.called, f"Cache policy '{cache_policy}' should have called export_raw"
                    else:
                        assert mock_func.call_count == 0, f"Cache policy '{cache_policy}' should NOT have called extract_func"
