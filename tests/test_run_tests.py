...
class TestRunTests:
    ...
    def test_run_tests_default_all(self, mock_run):
        ...
        expected_cmd = [
            "python", "-m", "pytest",
            "--cov=pythoneeg", "--cov-report=term-missing", "--cov-report=html",
            "tests/"
        ]
...
class TestRunLinting:
    ...
    def test_run_linting_success(self, mock_run):
        ...
        expected_cmd = ["flake8", "pythoneeg/", "tests/"]
...
class TestRunTestsIntegration:
    ...
    def test_help_output(self):
        ...
        result = subprocess.run(
            [sys.executable, "tests/run_tests.py", "--help"],
            capture_output=True,
            text=True
        )
        
        assert result.returncode == 0
        assert "Run PythonEEG tests" in result.stdout
